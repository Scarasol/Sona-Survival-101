...
public class Corrosion extends PhysicalEffect{
    ...
    public void applyEffectTick(LivingEntity entity, int amplifier) {
        entity.hurt(SonaDamageTypes.damageSource(entity.level().registryAccess(), SonaDamageTypes.CORROSION), (float) (equipmentCorroded(entity, amplifier) * (amplifier + 1) / (0.2 * (amplifier + 1) + 1)));
        if (entity.hasEffect(SonaMobEffects.SLIMINESS.get())){
            ...
            entity.level().explode(null, entity.getX(), entity.getY(), entity.getZ(), (amplifier + 1 + level) / 2, Level.ExplosionInteraction.NONE);
        }
    }
